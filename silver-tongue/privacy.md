# Privacy Policy — Silver Tongue

**Effective date:** 2026-05-29
**Publisher:** Infinity Games
**Contact:** ai.infinitygames@gmail.com

This is the privacy policy for the mobile application **Silver Tongue** ("the App"), published by Infinity Games ("we", "us", "our"). This policy explains what data the App collects, how we use it, and which third parties receive it.
...
- Validate that you watched an ad before granting an energy refill (single-use ad reward token)
- Track your subscription status if you purchase Premium
- Record your Daily Decree score on the global leaderboard (the leaderboard displays an anonymous alias derived from your device ID — the raw ID is never shown publicly)
- Keep your gold, energy, and progress (which opponents you have cracked) so they persist across sessions
- Enforce the once-per-day free Daily Decree attempt and a per-day generation cap to prevent abuse
...
### 1.2 Conversation transcripts (during a duel)
While a duel is in progress, the messages you send and the AI opponent's replies are stored temporarily on our backend so the AI can continue the conversation in context. These transcripts are automatically deleted after 6 hours (or when the duel ends, whichever is sooner). They are keyed by an opaque conversation ID associated with your device ID; they are never combined with information from outside the App and never shared with advertisers.

### 1.3 Subscription receipts
If you subscribe to Premium, the Google Play purchase token is sent to RevenueCat (our subscription validator) and to our backend solely to confirm that your subscription is active. We store the expiry timestamp keyed by your device ID.

### 1.4 Local app state
The App stores a small amount of data on your device only (never sent to our backend):

- Whether you have seen the "How to Play" introduction
- The opaque device ID itself
...
|---|---|---|
| **Google AdMob** | Showing rewarded ads to refill energy | Android Advertising ID, IP address, device model, ad interactions. AdMob is used in compliance with Google's [Families Policy](https://support.google.com/googleplay/android-developer/answer/9893335). [AdMob privacy policy](https://policies.google.com/privacy) |
...
| **OpenRouter / Google Gemini** | Generating the AI opponent's persona and replies during a duel | The text of your messages within a duel, plus the daily opponent's persona prompt. Never your device ID or any identifying information. [OpenRouter privacy](https://openrouter.ai/privacy) |
...
- **Energy / gold / "solved" progress**: kept on our backend keyed by device ID until you uninstall (no automatic expiry).
- **Conversation transcripts**: 6 hours, then automatically deleted.
- **Daily Decree leaderboard scores**: 7 days, then automatically deleted.
- **Daily Decree attempt counter**: 48 hours (covers the UTC day plus clock skew), then automatically deleted.
- **Generated-opponent cache** (daily opponent persona): 48 hours, no per-user identifying information.
...
Data in transit to our backend uses TLS (HTTPS). Subscription, ad-reward, and gameplay records are stored in Cloudflare KV under access keys we control. We do not transmit purchase tokens to any party besides RevenueCat (for validation) and our backend.
